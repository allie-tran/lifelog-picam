...
from schemas.general import LifelogImage
...
from services.face_recognition import add_face_to_whitelist, search_for_faces
...
@router.put("/set-recognition-mode")
...
@router.get("/all-device-settings")
...
@router.post("/get-faces", response_model=List[LifelogImage])
...
@router.get("/get-whitelist", response_model=List[WhitelistEntry])
def get_whitelist(
    device: str,
    access_level: Annotated[AccessLevel, Depends(auth_dependency)] = AccessLevel.NONE,
    session: Session = Depends(get_session),
...
@router.delete("/remove-from-whitelist")
