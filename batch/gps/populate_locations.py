...
class SegmentIndex:
    ...
    def find(self, ts: float) -> dict | None:
        ...
        if idx + 1 < len(self.segs):
            candidates.append((abs(ts - self.segs[idx + 1]["start_ts"]), self.segs[idx + 1]))
...
def fetch_unenriched(col, run_id) -> list[dict]:
    ...
    return list(col.find(
        {**BASE_QUERY, "$or": [{"location": {"$exists": False}}, {"location": None}, {"location.run_id": {"$ne": run_id}}]},
        {"_id": 1, "timestamp": 1},
        sort=[("timestamp", 1)],
    ))
...
def run(segments_file: str, mongo_uri: str, dry_run: bool):
    ...
    for i, seg in tqdm(enumerate(segments, 1), total=len(segments), desc="Processing segments"):
        seg["run_id"] = run_id
    ...
    for img in tqdm(unenriched, desc="Assigning fallback segments"):
        ...
        ok = col.update_one({"_id": img["_id"]}, {"$set": {"location": seg}}).modified_count == 1
        total_matched  += 1
        total_modified += int(ok and not dry_run)
 
    print(f"  ── Pass 2 done  matched={total_matched}"
